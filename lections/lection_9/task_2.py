...
# hello и bye и поместили в них результат работы функции add_one_str с
# разными аргументами. Теперь мы можем вызывать новые функции и получать
# объединённые строки передавая только окончание. Первая часть строки
# оказалась замкнута в локальной области видимости. И у каждой из двух новых
# функций область своя и начало строки своё.
...
# разными экземплярами функции add_two_str. Id, т.е.адреса в оперативной
# памяти разные, а названия указывают на оригинал.