...
# Переименование файла подразумевает сохранение неизменного файла в
# файловой таблице и в содержимом файле, но изменение его имени. Для
# переименования можно воспользоваться следующим кодом.
...
# было занято. Модуль pathlib позволяет сделать переименование через
# создание экземпляра класса Path с указанием исходного файла. А затем
# вызов метода rename задаёт новое имя. При этом операции могут быть
# объединены в одну строку через точечную нотацию и без явного сохранения
# экземпляра класса в переменной.
...
# Также при работе через os мы присвоили файлу новое имя. А при
# использовании модуля pathlib указали, что после переноса файл должен
# сохранить старое имя.
...
# функция попытается присвоить копируемому файлу имя “цели”. Отличие
# состоит в том, что функция copy2 помимо содержимого файла пытается
